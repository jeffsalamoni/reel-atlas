...
// Expedia affiliate tracking params (from your Creator Toolbox links)
...
// ── Expedia Affiliate Link Builder ─────────────────────
// Uses the Partnerize affiliate?camref= format which supports
// fully dynamic landing page URLs — no pre-generated slugs needed.
function buildAffiliateLinks(city, lat, lon) {
  ...
  // Hotel search — destination parameter pre-fills the city
  const hotelDest = encodeURIComponent(`https://www.expedia.com/Hotel-Search?destination=${encodeURIComponent(city)}`);

  // Car search — locn + coordinates for precise location matching
  const carParams = `locn=${encodeURIComponent(city)}&olat=${lat}&olon=${lon}`;
  const carDest   = encodeURIComponent(`https://www.expedia.com/carsearch?${carParams}`);
...
async function getPlaceDetails(query) {
  ...
  // Step 1: Text Search → name, coords, rating, types, address, place_id
  ...
  // Step 2: Place Details → address_components for accurate city name
